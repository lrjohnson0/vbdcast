...
preds.totalcases <- function(data, R0, f=rep(1/10, 10), fdy=rep(1/4,4))
  {
    ...
    if(any(is.na(Xy.raw$ndvi50))) stop("missing value in raw NDVI series")

    ## filter the predictors
    Xy.smooth <- Xy.raw
    for(nm in names(Xy.raw)) {
      if(is.element(nm, c("y", "season", "w", "epi"))) next; ##, "lgm", "gm"
    ...
    if(any(is.na(Xy.smooth$ndvi50))) stop("missing value in smoothed NDVI series")
    ...
    Xy <- data.frame(
                     ...
                     lpop.m1=Xy.smooth$lpop[(l.lim):(n-1)],
                     ...
                     tavg2.m11=(Xy.smooth$tavg[(l.lim-10):(n-1-10)])^2,
                     ndvi45.m1=Xy.smooth$ndvi45[l.lim:(n-1)], ## missing data
                     ndvi45.m16=Xy.smooth$ndvi45[(l.lim-15):(n-16)], ## missing data
                     ndvi50.m1=Xy.smooth$ndvi50[l.lim:(n-1)], ## missing data
                     ndvi50.m11=Xy.smooth$ndvi50[(l.lim-10):(n-11)], ## missing data
                     R0.m1=Xy.smooth$R0[l.lim:(n-1)],
                     R0.m10=Xy.smooth$R0[(l.lim-9):(n-10)],
...
fit.totalcases <- function(data, f=rep(1/10, 10))
{
  ...
  fit.bic <- step(fit, scope=list(upper=formula(fit), lower=formula(fit.m)),
...
fit.totalcases.nb <- function(data, f=rep(1/10, 10), plots=FALSE)
{
  R0 <- build.R0(data)
